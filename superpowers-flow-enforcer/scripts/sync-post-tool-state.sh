...
if [ "$TOOL_NAME" = "AskUserQuestion" ]; then
  update_state '.current_phase = "brainstorming" | .brainstorming.questions_asked = ((.brainstorming.questions_asked // 0) + 1) | .brainstorming.findings_updated = false'
fi
...
if [ "$TOOL_NAME" = "Write" ] || [ "$TOOL_NAME" = "Edit" ]; then
  ...
  if [ -n "$FILE_PATH" ]; then
    if [ "$(basename "$FILE_PATH")" = "findings.md" ]; then
      jq --arg now "$NOW_UTC" '.current_phase = "brainstorming" | .brainstorming.findings_updated = true | .brainstorming.findings_last_update = $now' "$STATE_FILE" > "$tmp_file"
      mv "$tmp_file" "$STATE_FILE"
    fi
...
if [ "$TOOL_NAME" = "Bash" ]; then
  ...
  if echo "$COMMAND" | grep -q 'git worktree add'; then
    ...
    jq --arg path "$WORKTREE_PATH" '.current_phase = "worktree" | .worktree.created = true | .worktree.path = $path' "$STATE_FILE" > "$tmp_file"
    mv "$tmp_file" "$STATE_FILE"
  fi
  ...
  if echo "$COMMAND" | grep -qiE '(^| )(npm|pnpm|yarn|bun|pytest|cargo test|go test|vitest|jest)( |$)'; then
    if echo "$RESULT_TEXT" | grep -qiE 'pass|passed|ok'; then
      update_state '.current_phase = "tdd" | .worktree.baseline_tests_passed = true'
    fi
