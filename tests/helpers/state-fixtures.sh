...
write_v2_state() {
  cat > "$1" <<'EOF'
{"state_version":2,"current_phase":"init","brainstorming":{"question_asked":false,"findings_updated_after_question":false,"spec_written":false,"spec_file":null,"spec_reviewed":false,"user_approved_spec":false},"planning":{"plan_written":false,"plan_file":null,"plan_reviewed":false,"execution_mode":null},"worktree":{"created":false,"path":null,"baseline_verified":false},"workflow":{"active":false,"activated_by":null,"activated_at":null,"override":null,"deactivated_by":null,"deactivated_at":null},"task_flow":{"active_task_id":null,"active_packet_role":null,"last_dispatch_at":null},"resume":{"recovery_required":false,"recovery_completed_at":null,"last_resume_source":null},"tdd":{"current_task":null,"current_step":null,"pending_failure_record":false,"last_failed_command":null,"test_files_created":[],"production_files_written":[],"tests_verified_fail":[],"tests_verified_pass":[]},"review":{"tasks":{}},"finishing":{"invoked":false},"debugging":{"active":false,"phase":null,"fixes_attempted":0,"root_cause_found":false},"exceptions":{"skip_brainstorming":false,"skip_planning":false,"skip_tdd":false,"skip_review":false,"skip_finishing":false,"pending_confirmation_for":null,"reason":null,"user_confirmed":false,"confirmed_at":null},"interrupt":{"allowed":false,"reason":null,"keywords_detected":[]}}
EOF
}
...
write_v2_state_without_task_flow() {
  write_v2_state "$1"
  jq 'del(.task_flow)' "$1" > "$1.tmp"
  mv "$1.tmp" "$1"
}

write_v2_state_with_partial_task_flow() {
  write_v2_state "$1"
  jq 'del(.task_flow.active_packet_role, .task_flow.last_dispatch_at)' "$1" > "$1.tmp"
  mv "$1.tmp" "$1"
}

write_v2_state_with_invalid_task_flow_role_string() {
  write_v2_state "$1"
  jq '.task_flow.active_packet_role = "manager"' "$1" > "$1.tmp"
  mv "$1.tmp" "$1"
}

write_v2_state_with_invalid_task_flow_role_type() {
  write_v2_state "$1"
  jq '.task_flow.active_packet_role = {"unexpected":true}' "$1" > "$1.tmp"
  mv "$1.tmp" "$1"
}

write_v2_state_with_non_object_task_flow() {
  write_v2_state "$1"
  jq '.task_flow = "broken"' "$1" > "$1.tmp"
  mv "$1.tmp" "$1"
}

write_v2_state_without_resume() {
  write_v2_state "$1"
  jq 'del(.resume)' "$1" > "$1.tmp"
  mv "$1.tmp" "$1"
}

write_v2_state_with_partial_resume() {
  write_v2_state "$1"
  jq 'del(.resume.recovery_completed_at, .resume.last_resume_source)' "$1" > "$1.tmp"
  mv "$1.tmp" "$1"
}
