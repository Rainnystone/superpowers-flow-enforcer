...
- **Manual Control**: Prefer `开启 enforcer` / `enable enforcer` to turn workflow enforcement on and `关闭 enforcer` / `disable enforcer` to turn it off. Long-form compatibility still exists for `激活 superpowers enforcer` / `activate superpowers enforcer` and `关闭 superpowers enforcer` / `deactivate superpowers enforcer`. `启动 enforcer`, `停止 enforcer`, `start enforcer`, and `stop enforcer` are NOT SUPPORTED in this round, and `stop` must not be interpreted as disabling enforcement.
