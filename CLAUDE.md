...
- **Workflow Entry**: The current implementation treats skip requests, the exact manual-control commands `开启 enforcer` / `enable enforcer`, the exact long-form compatibility commands `激活 superpowers enforcer` / `activate superpowers enforcer`, and canonical `docs/superpowers/specs/*.md` / `docs/superpowers/plans/*.md` writes within the current project scope as entry signals. Root-level canonical paths remain valid, subtree canonical paths such as `packages/foo/docs/superpowers/specs/*.md` also activate, and excluded trees like `.git`, `.worktrees`, `node_modules`, `vendor`, `.simulation`, and fixture/testdata do not.
...
- **Requirements**: Node 18+, `jq`, and a Python runtime resolvable as `python3` or `python`.
...
- **One-Command Test Entry**: From the repo root, run `bash scripts/run-test-suite.sh` to execute every `tests/test_*.sh` script.
